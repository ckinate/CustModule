...
namespace Fintrak.VendorPortal.Blazor.Client.Onboarding.Models.Validators
{
	public class OfficialInformationValidator : AbstractValidator<OfficialInformationModel>
	{
		public OfficialInformationValidator(OnboardingStepService onboardingStepService)
		{
		 ...
		 @".)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$").WithMessage("{PropertyName} not valid.").When(c => onboardingStepService.CurrentStep == 1);
			...
			RuleFor(p => p.CategoryId)
				.Must(c => c > 0).WithMessage("Category is required.").When(c => onboardingStepService.CurrentStep == 1);

			//RuleFor(p => p.CategoryName)
			//	.NotEmpty().WithMessage("{PropertyName} is required.");

			RuleFor(p => p.SubCategoryIds)
				.Must(c => c!= null && c.Length > 0).WithMessage("Sub Category(s) is required.").When(c => onboardingStepService.CurrentStep == 1);

			//RuleFor(p => p.SubCategoryNames)
		...
		//	{
		//		RuleFor(p => p.Id)
		//		.NotNull().WithMessage("Sub Category is required.")
