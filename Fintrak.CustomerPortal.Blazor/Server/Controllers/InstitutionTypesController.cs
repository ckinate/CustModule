...
namespace Fintrak.CustomerPortal.Blazor.Server.Controllers
{
    public class InstitutionTypesController : ApiControllerBase
	{
		...
		public InstitutionTypesController(IWebHostEnvironment webHostEnvironment, IConfiguration configuration, ICustomerIntegrationService customerIntegrationService)
		{
        ...
        [HttpGet("lookup")]
        public async Task<ActionResult<BaseResponse<List<LookupModel<string, int>>>>> GetLookup([FromQuery]int? parentId, [FromQuery] bool? parentOnly)
        {

            var response = new BaseResponse<List<LookupModel<string, int>>> { Result = new List<LookupModel<string, int>>() };

            var entities = await GetInstitutionTypes(parentId, parentOnly);
        ...
        private async Task<List<LookupModel>> GetInstitutionTypes(int? parentId, bool? parentOnly = false)
        {
            ...
            if (useOfflineData)
            {
                ...
                var fullPath = Path.Combine(rootPath, "JsonData/InstitutionTypes.json");
            ...
            {
                var result = await _customerIntegrationService.GetInstitutionTypeLookUp(parentId, parentOnly, false);
